...
export async function GET(request: Request, context: RouteContext) {
  ...
  console.log("[PDF_DOWNLOAD_REDIRECT_ROUTE_HIT]", { accessToken });
  ...
  try {
    ...
    console.log("[PDF_DOWNLOAD_SIGNED_URL_READY]", {
    ...
    console.log("[PDF_DOWNLOAD_REDIRECTING]", {
      bookId: storedBook.id,
    });

    return NextResponse.redirect(signedUrl, { status: 302 });
  } catch (error) {
    console.error("[PDF_DOWNLOAD_REDIRECT_FAILED]", {
      accessToken,
